...
# 1. dataset
from src.dataset.uav_flow import UAV_Dataset
...
def main(args):
    ...
    ######################################
    # load data
    if args.dataset["name"] == "uav_flow":
        train_dataset = UAV_Dataset(
    ...
    for epoch in range(EPOCH):
        ...
        if args.dataset["name"] == "uav_flow":
            ...
            L2_ps = gather_tensor(torch.tensor(train_error['L2_ps'], device=local_rank), world_size)
            L2_pg = gather_tensor(torch.tensor(train_error['L2_pg'], device=local_rank), world_size)
            ...
            if local_rank == 0:
                ...
                print(f"L2 loss: u {L2_u:.4e}, v: {L2_v:.4e}, ps: {L2_ps:.4e}, pg: {L2_pg:.4e}, train_mean_l2: {train_mean_l2:.4e}")
                print(f"each time step loss: {each_t_l2}")
                print(f"RMSE loss: U {RMSE_u:.4e}, P: {RMSE_p:.4e}")
                ...
                writer.add_scalar('L2/train_L2_ps', L2_ps, epoch)
                writer.add_scalar('L2/train_L2_pg', L2_pg, epoch)
                ...
                with open(f"{args.save_path}/record/{args.name}_training_log.txt", "a") as file:
                    ...
                    file.write(f"L2 loss: u {L2_u:.4e}, v: {L2_v:.4e}, ps: {L2_ps:.4e}, pg: {L2_pg:.4e}, train_mean_l2: {train_mean_l2:.4e}\n")
                    file.write(f"each time step loss: {each_t_l2}\n")
                    file.write(f"RMSE loss: U {RMSE_u:.4e}, P: {RMSE_p:.4e}\n")
        ...
        if (epoch+1) % 5 == 0 or epoch == 0 or (epoch+1) == EPOCH:
            ...
            if args.dataset["name"] == "uav_flow":
                ...
                if local_rank == 0:
                    ...
                    writer.add_scalar('L2/test_L2_ps', test_L2_ps, epoch)
                    writer.add_scalar('L2/test_L2_pg', test_L2_pg, epoch)
