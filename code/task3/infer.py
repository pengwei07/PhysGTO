...
from thop import profile  # 方法1: 使用thop
    ...
    with torch.no_grad():
        ...
        for i, [batch_data, idx] in enumerate(test_dataloader):
            ...
            for split_id, split_data in splits_data.items():
                ...
                flops = macs * 2  # MACs通常是FLOPs的一半
                print(f"使用thop计算 - 参数量: {params/1e6:.2f}M, macs: {macs/1e9:.2f}G, FLOPs: {flops/1e9:.2f}G")
            ...
            # 保存数据到h5文件中，使用 idx 作为 key
            # key = str(idx[0].item())
            
            # # 创建每个idx对应的group
            # grp = h5file.create_group(key)
            
            # # 保存ground truth、prediction和loss
            # grp.create_dataset('ground_truth', data=ori_pressre.cpu().numpy())
            # grp.create_dataset('prediction', data=final_result.cpu().numpy())
            # grp.attrs['l2_loss'] = l2_loss.item()
